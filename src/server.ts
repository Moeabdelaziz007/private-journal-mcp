  ...
  private setupToolHandlers(): void {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        {
          name: 'analyze_knowledge_topology',
          description: 'Analyze the knowledge graph to find connections between concepts.',
          inputSchema: {
            type: 'object',
            properties: {
              conceptA: {
                type: 'string',
                description: 'The first concept to analyze.',
              },
              conceptB: {
                type: 'string',
                description: 'The second concept to analyze.',
              },
            },
        ...
        },
        {
          ...
          description: 'Explore multiple solutions for a problem simultaneously.',
          inputSchema: {
            type: 'object',
            properties: {
              problem: {
              ...
              solutions: {
                type: 'array',
                items: { type: 'string' },
                description: 'A list of potential solutions to explore.',
              },
            },
        ...
        {
          name: 'find_entangled_concepts',
          description: 'Find non-obvious connections to a concept.',
          inputSchema: {
            type: 'object',
            properties: {
              concept: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (request.params.name === 'analyze_knowledge_topology') {
        if (!args || typeof args.conceptA !== 'string' || typeof args.conceptB !== 'string') {
          throw new Error('conceptA and conceptB are required and must be strings');
        }
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
          throw new Error(`Failed to analyze knowledge topology: ${errorMessage}`);
        }
      }

      if (request.params.name === 'explore_solutions') {
        if (!args || typeof args.problem !== 'string' || !Array.isArray(args.solutions)) {
          throw new Error('problem (string) and solutions (array) are required');
        }
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
          throw new Error(`Failed to explore solutions: ${errorMessage}`);
        }
      }

      if (request.params.name === 'find_entangled_concepts') {
        if (!args || typeof args.concept !== 'string') {
          throw new Error('concept is required and must be a string');
        }
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
          throw new Error(`Failed to find entangled concepts: ${errorMessage}`);
        }
      }
